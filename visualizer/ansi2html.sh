...
<div style="float:left;width:40%;height:100%;overflow:scroll;">
<pre>
'
...
ansi2html --palette=solarized > /tmp/tmphtml
cat /tmp/tmphtml | tr '\n' '\r' | sed 's/<pre>/<pre>\n/g' | sed 's:</pre>:\n</pre>:g' > /tmp/tmphtml2
tail -n 2 /tmp/tmphtml2 | head -n 1 | sed 's:\r:</div>\n:g' | sed 's:^</div>$::g' | sed 's/^\([^$]\)/<div id="lineLLLLL">\1/g' | grep -v '^$' > /tmp/tmphtml3
rm -f /tmp/tmphtml4
n=0
while read line
do
	n=$((++n))
	echo -n $line | sed -e 's/LLLLL/'$(($n))'/' >> /tmp/tmphtml4
done < /tmp/tmphtml3

head -n 1 /tmp/tmphtml2; cat /tmp/tmphtml4; tail -n 1 /tmp/tmphtml2
