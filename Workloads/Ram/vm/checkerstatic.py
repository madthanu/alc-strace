...
mountandcopycommand ='''rm -f /tmp/blah;sudo LD_LIBRARY_PATH=/usr/lib/vmware-vix-disklib/lib64 vmware-mount -X;cd /home/ramnatthan/workload_snapshots/vm/replayedsnapshot;
						...
						echo -------------------;sudo LD_LIBRARY_PATH=/usr/lib/vmware-vix-disklib/lib64 vmware-mount -f ./testvm-static.vmdk /mntpt/ 2>> /tmp/short_output;echo -------------------;sudo LD_LIBRARY_PATH=/usr/lib/vmware-vix-disklib/lib64 vmware-mount -L;
...
with open('/tmp/short_output', 'a') as fout:
	if not os.path.exists('/tmp/blah'):
		...
		recovery_command = '''sudo LD_LIBRARY_PATH=/usr/lib/vmware-vix-disklib/lib64 vmware-vdiskmanager -R /home/ramnatthan/workload_snapshots/vm/replayedsnapshot/testvm-static.vmdk 2>>/tmp/short_output'''
