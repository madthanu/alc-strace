...
class BerkeleyDBWorkload:
    ...
    def finddurabilitysignal(self, keyword, stdoutlist):
	for msg in stdoutlist:
		if keyword in msg:
			return True
	return False
    ...
    def GetDBInstance(self, recover):
        ...
        if(self.useEnv):
            ...
            self.env.set_flags(db.DB_CREATE | db.DB_CHKSUM, 1)
    ...
    def RunRec(self, op, numTuples = 4, rec = False, dbverifyOutput = '', args = []):
        ...
        if dbvar is not None:
            try:
                dbvar.open(self.dbLocation + '/' + self.fileName, None, self.accessMethod, db.DB_CREATE | db.DB_INIT_LOG | db.DB_INIT_TXN)
            ...
            if not proper:
                ...
                if killed:
                    ...
                    if exc == 'None':
                    	if self.finddurabilitysignal('TXNCommitDone', args):
                    		if cc != 1200:
	                        	fo.write(prefix + 'WOR: Durability signal present. But No of rows retrieved:' + str(cc))
	                        else: 
	                        	fo.write(prefix + 'WOR: Durability signal present. No problem. No of rows retrieved:' + str(cc))
	                else:
	                    	if cc == 1 or cc == 1200:
	                    		fo.write(prefix + 'WOR: Durability signal absent. No problem. No of rows retrieved:' + str(cc))
	                    	else:
	                    		fo.write(prefix + 'WOR: Durability signal absent. Second case  - But No of rows retrieved:' + str(cc))    	
        ...
        if not proper:
        	...
        	if self.finddurabilitysignal('TXNCommitDone', args):
        		if ccwr != 1200:
                		fo.write(prefix + 'TWR: Durability signal present. But No of rows retrieved:' + str(ccwr))
                	else: 
                		fo.write(prefix + 'TWR: Durability signal present. No problem. No of rows retrieved:' + str(ccwr))
                else:
            		if ccwr == 1 or ccwr == 1200:
            			fo.write(prefix + 'TWR: Durability signal absent. No of rows retrieved:' + str(cc))
            		else:
            			fo.write(prefix + 'TWR: Durability signal absent. Second case  - But No of rows retrieved:' + str(ccwr))    	
...
workload1.RunRec(op = 'display', numTuples = 1200, rec = rec, dbverifyOutput = out, args = sysargs)
