...
class BerkeleyDBWorkload:
    ...
    def GetDBInstance(self):
        ...
        if(self.useEnv):
            ...
            self.env.set_flags(db.DB_CREATE | db.DB_NOMMAP |  db.DB_CHKSUM, 1) #include NO_MMAP if needed
    ...
    def Run(self, op, numTxn, numTuples = 4, syncFlag = db.DB_TXN_SYNC):
        ...
        if op == 'insert':
            ...
            #print 'explicitly enable checksums'
            ...
            if self.useEnv:
                if self.useAutoCommit:
                    dbvar.open(self.dbLocation + '/' + self.fileName, None, self.accessMethod, db.DB_CREATE | db.DB_AUTO_COMMIT | db.DB_NOMMAP) #include NO_MMAP if needed
                else:
                    dbvar.open(self.dbLocation + '/' + self.fileName, None, self.accessMethod, db.DB_CREATE | db.DB_INIT_LOG | db.DB_INIT_TXN | db.DB_NOMMAP) #include NO_MMAP if needed
            else:
                dbvar.open(self.dbLocation + '/' + self.fileName, None, self.accessMethod, db.DB_CREATE | db.DB_NOMMAP) #include NO_MMAP if needed
...
workload1 = BerkeleyDBWorkload( dbLocation = '/home/ramnatthan/code/adsl-work/ALC/bdb/databases', accessMethod = db.DB_HASH,
                                fileName = 'mydb.db', useEnv = True,
                                cleanupBeforeRun = True, sync = True, useAutoCommit = True)

workload1.Run(op = 'insert', numTxn =1, numTuples = 1, syncFlag = db.DB_TXN_SYNC)
